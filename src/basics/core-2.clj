...
(def more-let (let [[first & rest] ["jon" "sean" "tap"]] [first rest]))

(def numb-x 100)
(defn mains
  []
  (println (loop [iteration 5]
             (println (str "iter: " iteration))
             (if (< iteration 1)
               (println "bye!")
               (recur (dec iteration))
               ))))
(def starting-match-text (re-find #"^what" "what-day"))
(def profile {:name "john" :age 90})
(defn matching-part
  [part]
  {:name (clojure.string/replace (:name part) #"john" "peter")
   :age (:age part)}
  )
(defn main-old-1
  []
  (println (matching-part {:name "john" :age 900})))
(defn -main-23
  []
...
;;to find matching starting text
;;(re-find #"^starting" "in-here")s

;;reduce
;;(reduce + [1 2 3])
;; with initial value (reduce  + 10 [1 3])